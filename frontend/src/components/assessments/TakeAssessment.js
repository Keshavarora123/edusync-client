    ...
    try {
      ...
      const submission = {
        ...
        answers: Object.entries(answers).map(([questionId, optionId]) => {
          const question = assessment.questions.find(q => q.questionId === parseInt(questionId));
          if (!question) {
        ...
        timeSpentSeconds: assessment.timeLimitMinutes
          ? (assessment.timeLimitMinutes * 60 - timeLeft)
          : null
      };
      ...
      // Submit to the API
      const response = await fetch(`${config.apiBaseUrl}/Assessments/${assessmentId}/submit`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user.token}`,
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = errorData.message || `Failed to submit assessment: ${response.status} ${response.statusText}`;
        } catch (parseError) {
          console.error('Error parsing error response:', parseError);
          errorMessage = `Failed to submit assessment: ${response.status} ${response.statusText}`;
        }
      ...
      // Redirect to course assessments page
